...
<!--	<input type="radio" group="opts" name="auth_opts" value="uauth" <c:if test='${ (!empty param.auth_opts) && param.auth_opts eq "uauth" }'>checked</c:if> /> Disambiguated Search &#124;
	<input type="radio" group="opts" name="auth_opts" value="ic" <c:if test='${ (!empty param.auth_opts) && param.auth_opts eq "ic" }'>checked</c:if> /> Include Citations &#124; -->
